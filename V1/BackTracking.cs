...
class algo_ResolutionBackTracking
{
    ...
    */
    public static void ResousSudoku(int[,] Xgrille)
    {
        ...
        else
        {
            ...
            // Si la fonction a renvoyé true, alors le programme a trouvé la solution donc on l'affiche
            if (resolution)
            {
                Console.WriteLine();
                Console.WriteLine("Voici la grille résolue : ");
                affiche_grille(Xgrille);
            }
            // Si la grille n'est pas résolu par la fonction, alors celle-ci n'est pas solvable
            else
            {
    ...
    */
    public static bool ResolutionBackTracking(int[,] Xgrille)
    {
        ...
        int ancienne_valeur_de_la_case = 0;     // Cette variable nous servira à stocker l'ancienne valeur qu'il y avait dans une case avant de revenir dessus, afin de tester les autres valeurs qui n'ont pas encore été testé 
        List<string> cases_remplies = new List<string>();   // Liste contenant toutes les cases (ligne,colonne,valeur de la case) où nous avons inséré une valeur

        bool resolu = false;    // Ce booléen nous permettra d'arrêter si nous avons trouvé la solution
        bool impossible = false;    // Ce booléen nous permettra d'arrêter si nous avons testé toutes les possibilitées et nous n'avons trouvé aucune solution
        ...
        while (i < Xgrille.GetLength(0) && resolu == false && impossible == false)  // Parcourt de chaque ligne
        {
            while (j < Xgrille.GetLength(1) && resolu == false && impossible == false)  // Parcourt de chaque colonne
            {
                ...
                if (valeur_case == 0)       // Si la case est vide
                {
                    ...
                    bool nombre_a_inserer_trouve = false;      // Ce booléen nous permettra d'arrêter de chercher un candidat dès que nous en aurons trouvé un
                    
                    for (int nombre_a_inserer = 1; nombre_a_inserer <= Xgrille.GetLength(0) && nombre_a_inserer_trouve == false; nombre_a_inserer++)        // Parcourt de tous les nombres possibles sur cette grille
                    {
                        if (nombre_a_inserer > ancienne_valeur_de_la_case)      // Quand nous revenons à la case précédente car nous étions bloqués à l'étape précédente, il faut commencer à tester à partir de la valeur juste après l'ancien nombre qu'il y avait avant (par exemple, s'il y avait un 3 nous commencerons à 4)
                        {
                            ...
                            if (ligne_valide && colonne_valide && region_valide)        // Si le nombre est un candidat ( = s'il n'est pas présent sur la ligne, la colonne ou la région)
                            {
                    ...
                    {
                        if (cases_remplies.Count > 0)       // S'il y a au moins une case où nous avons émis une hypothèse --> on la remet à vide
                        {
                            ...
                            cases_remplies.RemoveAt(derniere_pos);  // Il faut enlever la case car on vient de l'utiliser pour revenir dessus
                            ...
                            Xgrille[i, j] = 0;      // Nous remettons la case à vide car l'ancienne valeur qu'il y avait dedans ne fonctionne pas
                        }
    ...
    */
    public static bool grille_complete(int[,] XXgrille)
    {
        ...
        bool trouve = false;        // Ce booléen nous servira à sortir du parcourt de la grille dès que nous trouverons UNE SEULE case vide
        ...
        for (int i = 0; i < XXgrille.GetLength(0) && trouve == false; i++)
        {
            ...
            {
                if (XXgrille[i, j] == 0)        // Si la case est vide, alors la grille n'est pas complète et nous avons trouvé la réponse
                {
                    res = false;
                    trouve = true;
                }
    ...
    */
    ...
    {
        bool res = true;    // De base, nous considérons que nous pouvons insérer l'entier (le nombre n'est pas présent sur la ligne)
        ...
        for (int col = 0; col < grille.GetLength(1) && trouve == false; col++)
        {
            ...
            if (!(col == Xj))
            {
                if (grille[Xi, col] == Xentier_verifier)    // Si la case de la ligne contient le nombre dont nous voulons vérifier la validité, alors nous ne pouvons pas l'insérer
                {
                    res = false;
                    trouve = true;
                }
    ...
    */
    ...
    {
        bool res = true;    // De base, nous considérons que nous pouvons insérer l'entier (le nombre n'est pas présent sur la colonne)
    ...
    public static bool verifie_region(int[,] grille, int Xi, int Xj, int Xentier_a_verifier)
    {
        ...
        for (double i = r.ligne_debut; i < r.ligne_fin; i++)
        {
            for (double j = r.colonne_debut; j < r.colonne_fin && trouve == false; j++)
            {
                ...
                if (i != Xi && j != Xj)
                {
                    int entier_actuel = grille[(int)i, (int)j];  // Nous devons caster car les indices des cases dans un tableau sont des entiers
    ...
    */
    public static bool repetition_nombre(int[,] Xgrille)
    {
        ...
        bool trouve = false;    // Dès que l'on trouvera une répétition d'un nombre, ce booléen nous permettra de casser la boucle
    ...
    */
    public static bool nombre_non_autorise(int[,] Xgrille)
    {
        ...
        bool trouve = false;    // Dès que l'on trouvera un nombre dépassant le nombre maximum autorisé, ce booléen nous permettra de casser la boucle
