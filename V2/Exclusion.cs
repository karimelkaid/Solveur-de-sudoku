...
class algo_resolution_partielle
{
...
*/
    ...
    */
    public static void ResousSudoku(int[,] Xgrille)
    {
        ...
        {
            ResousExclusion(Xgrille);

            Console.WriteLine();
            Console.WriteLine("Voci la grille complétée grâce à la méthode de l'exclusion :");
            affiche_grille(Xgrille);
        ...
        for (int i = 0; i < Xgrille.GetLength(0); i++)
        {
            for (int j = 0; j < Xgrille.GetLength(1) && trouve == false; j++)
            {
                ...
                if (Xgrille[i, j] != 0)
                {
                    ...
                    if (!(ligne_verifie && colonne_verifie &&
                          region_verifie)) // Si le nombre est répété sur la ligne ou la colonne ou la région
                    {
                        trouve = true;
    ...
    */
    public static void ResousExclusion(int[,] Xgrille)
    {
        bool resolu = false;
        bool blocage = false;
